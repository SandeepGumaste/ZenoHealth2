    ...
    <section className="p-lg-0 m-lg-0">
      <div className="container position-relative py-4">
        <div className={`${styles.filters} pe-4`}>
          <button className={`${styles.searchButton} rounded-circle`}>
            {" "}
            <i className="bi bi-search fs-4" />
          </button>
          <div className="d-flex flex-column flex-lg-row  rounded-4 gap-4 gap-lg-0 px-4 py-4 ">
            <div ref={locationRef} className="d-flex col-lg-4 pe-lg-3">
              <i className="bi bi-geo-alt fs-2 me-2 mt-2" />
              <div
                className={`${
                  showLocationSelector ? "rounded-bottom-0" : "rounded-bottom-3"
                } border rounded-top-3 px-2 py-3 position-relative w-100 ${
                  showLocationSelector && styles.locationActive
                }`}
                onClick={handleLocationSelectorClick}
              >
                <label
                  className={`${styles.formLabel} bg-white px-2 position-absolute`}
                >
                  Location
                </label>
            ...
            <div ref={calendarRef} className="d-flex col-lg-4 px-lg-3">
              <i className="bi bi-calendar fs-2 me-2 mt-2" />
              <div
                className={`${
                  showDateSelector ? "rounded-bottom-0" : "rounded-bottom-3"
                } ${
                  showDateSelector && styles.calActive
                } border rounded-top-3 px-2 py-3 position-relative w-100`}
              ...
              >
                ...
                <span className="ps-2">{`${format(
                  range.startDate,
                  "dd MMM"
                )} to ${format(range.endDate, "dd MMM")}`}</span>
                {showDateSelector && (
                  <div>
                    <DateRangePicker
            ...
            <div className="d-flex col-lg-4 ps-lg-3">
              <i className="bi bi-person fs-2 me-2 mt-2" />
              <div
                className={`${
                  showGuestSelector ? "rounded-bottom-0" : "rounded-bottom-3"
                } border rounded-top-3 px-2 py-3 position-relative w-100 ${
                  showGuestSelector && styles.guestActive
                }`}
                onClick={handleGuestsAndRoomsClick}
              >
                <label
                  className={`${styles.formLabel} bg-white px-2 position-absolute`}
                >
                ...
                <span className="ps-2">
                  {guestsAndRooms.totalGuests && guestsAndRooms.totalGuests}{" "}
                  {pluralUtil(guestsAndRooms.totalGuests, "Guest")} &{" "}
                ...
                {showGuestSelector && (
                  <div
                    ref={guestsRef}
                    className={`${styles.countersContainer} px-3 py-2`}
                  >
                    {Object.keys(guestsAndRooms)
                      .filter((item) => item !== "totalGuests")
                      .map((item) => {
                        return (
                          <CommonCounter
                            key={item}
                            count={guestsAndRooms[item]}
                            name={item}
